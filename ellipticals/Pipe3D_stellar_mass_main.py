...
if CLEAR_COLS:
    DRP_table = add_cols(DRP_table, ['sum_M_star', 'sum_M_star_err'])
...
for gal_ID in FILE_IDS:
    ...
    if DRP_table['Mvir'][i_DRP] > 0:
        ...
        M_star, M_star_err = sum_Pipe3D_mass(pipe3d_maps['sMass_density'])

        if TEXT_OUT:
            print(gal_ID)
            print('M_star: ', M_star, ' M_star_err: ', M_star_err)

        DRP_table['sum_M_star'][i_DRP] = M_star
        DRP_table['sum_M_star_err'][i_DRP] = M_star_err
