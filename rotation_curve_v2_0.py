...
#    plt.close()
    ...
    #
    # Due to the data mask, sometimes attempting to access 'x_center' and
    #    'y_center' result in an IndexError. The try-except statement below
    #    deals with this error. In addition, the boolean 'unmasked_data' is
    #    set according to the outsome of the try-except statement and the rest
    #    of 'calc_rot_curve()' is then executed accordingly.
    #--------------------------------------------------------------------------
    ...
    ###########################################################################
    # If the 'unmasked_data' variable is set to True (i.e. the data array has
    #    unmasked data points), execute the function as normal. Otherwise, the
    #    try-except statement encountered an IndexError and each of the values
    #    will be set to -1 (see below).
    # &   &   &   &   &   &   &   &   &   &   &   &   &   &   &   &   &   &   &
    if unmasked_data:
        #######################################################################
        # Subtract the systemic velocity from data points without the mask and
        #    then multiply the velocities by sin( inclination angle) to account
        #    for the galaxy's inclination affecting the rotational velocity.
        #----------------------------------------------------------------------
        sys_vel = masked_Ha_vel[ y_center, x_center]
        inclination_angle = np.arccos( axes_ratio)

        masked_Ha_vel = masked_Ha_vel - sys_vel
        masked_Ha_vel /= np.sin( inclination_angle)
        #######################################################################


        #######################################################################
        # Find the global max and global min of 'masked_Ha_vel' to use in
        #    graphical analysis.
        #
        # NOTE: If the entire data array is masked, 'global_max' and
        #       'global_min' cannot be calculated. The above try-except
        #       statement was originally used to mitigate these situations, but
        #       it has been found that if the 'inclination_angle' is 0 degrees,
        #       the entire data array is then masked. An if-statement tests if
        #       this is the case, and sets 'unmasked_data' to False if there is
        #       no max/min in the array.
        #----------------------------------------------------------------------
    ...
    ###########################################################################
    # If 'unmasked_data' was set to False by either all of the 'v_band' data
    #    being masked or all of the 'Ha_vel' data being masked after correcting
    #    for the angle of inclination, append -1s to all of the data arrays and
    #    skip to creating the astropy Column objects for the astropy QTable.
    #--------------------------------------------------------------------------
...
#        print("phi:", phi_deg)
        ...
        #----------------------------------------------------------------------
        flux_center = masked_v_band[ y_center, x_center] \
                                        * ( u.erg / ( u.s * u.cgs.cm**2))
        flux_center_error = masked_v_band_err[ y_center, x_center] \
                                        * ( u.erg / ( u.s * u.cgs.cm**2))
...
#            plt.close()
    ...
    #
    # NOTE: If 'unmasked_data' was set to False in the try-except statement
    #       mentioned above, there will be no data to display in
    #       'vel_contour_plot.' Because of this, an if-statement is included
    #       to test for 'unmasked_data' in plotting 'vel_field_collected_fig.'
    # -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -
    if unmasked_data:
        vmax_bound = -1 * min( global_max, global_min)
        vmin_bound = min( global_max, global_min)
        ...
        vel_field_collected_fig = plt.figure( figsize=(6, 6))
...
#    plt.show()
    ...
    # -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -
    mass_curve_fig = plt.figure( figsize=(5, 5))
