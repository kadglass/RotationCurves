...
import sys
sys.path.insert(1, '/Users/kellydouglass/Documents/Research/DESI/Targets/RotationCurves/')
...
DESI_FILENAME = DESI_DIRECTORY + 'MaNGA_DESI_galaxies_08042021.txt'
...
for i in range(len(DESI_table)):
    gal_ID = DESI_table['plateifu'][i]
...
for gal_ID in FILE_IDS:
    ...
    if DRP_table['mngtarg1'][i_DRP] > 0:
        ...
        if map_smoothness <= map_smoothness_max or map_smoothness_5sigma <= map_smoothness_max:
            ####################################################################
            # Locate object in DESI table
            #-------------------------------------------------------------------
            i_DESI = DESI_index[gal_ID]
            ####################################################################
            ...
            #-------------------------------------------------------------------
            axis_ratio = DESI_table['BA'][i_DESI]
            phi_EofN_deg = DESI_table['PA'][i_DESI]
